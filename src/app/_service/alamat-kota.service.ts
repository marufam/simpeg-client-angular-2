...
@Injectable()
export class AlamatKotaService {
  ...
  getAll(prov){
    ...
    let option = { headers : headers};
  	...
  	return  this.http.get(env.url + this.myUri+ 'provinsi/'+prov+'/pages/1', option )
  	.map(res=>res.json())
  }
  ...
  getByProvinsi(param){
    ...
    return  this.http.get(env.url + this.myUri+ "/provinsi/" + param, option )
    .map(res=>res.json())
  }
